		...
		<div class="vamtam-ts">
			...
			<?php if ( $valid_key ) : ?>
				<div class="vamtam-plugin-manager vamtam-has-required-plugins">
					<h2><?php esc_html_e( 'Recommended Plugins', 'skole' ); ?></h2>
					...
					if ( count( $plugins['all'] ) ) {
						?>
						<p><?php echo sprintf( esc_html( 'You can install exclusive and recommended plugins for %s here, and add or remove them later on WordPress plugins page.', 'skole'), $theme_name ); ?></p>
								...
								<thead>
									<tr>
										<td id="cb" class="manage-column column-cb check-column" width="10%">
											...
											for="cb-select-all"><?php esc_html_e( 'Select All', 'skole' ); ?></label>
											<input
											id="cb-select-all" type="checkbox">
										</td>
										<th class="manage-column column-thumbnail"></th>
										<th scope="col" id="name"
										class="manage-column column-name" width="15%"><?php esc_html_e( 'Name', 'skole' ); ?></th>
										<th scope="col" id="description"
										class="manage-column column-description" width="50%"><?php esc_html_e( 'Description', 'skole' ); ?></th>
										<th scope="col" id="status"
										class="manage-column column-status" width="17%"><?php esc_html_e( 'Status', 'skole' ); ?></th>
										<th scope="col" id="version"
										class="manage-column column-version" width="8%"><?php esc_html_e( 'Version', 'skole' ); ?></th>
									</tr>
								</thead>
									...
									?>
										<tr class="vamtam-plugin" data-slug="<?php echo esc_attr( $slug ); ?>" data-category="<?php echo esc_attr( $plugin['category'] ); ?>">
											...
											<?php
												...
												if ( strpos( $slug, 'skole' ) !== false ) {
													echo '<div class="vamtam-label">' . esc_html__('Exclusive', 'skole') . '</div>';
												}
											?>
											</td>
												...
												<span>
													<?php
														if ( isset( $plugins['install'][ $slug ] ) ) {
															echo esc_html__( 'Not Installed', 'skole' );
														} elseif ( isset( $plugins['activate'][ $slug ] ) ) {
															echo esc_html__( 'Not Activated', 'skole' );
														}
													?>
												</span>
											</td>
						...
						<a id="install-plugins-btn" href="#"
							...
							data-callback="install_plugins"><?php esc_html_e( 'Install Plugins', 'skole' ); ?></a>
						</div>

					<?php
					} else { ?>
						<div class="vamtam-plugins-success">
							<?php echo esc_html__( 'All plugins are already installed and up to date.', 'skole'  ); ?>
						</div>
					<?php
					} ?>
				</div>
