...
function vamtam_register_required_plugins() {
	...
	$plugins = array(
		...
		array(
			'name'     => esc_html__( 'Elementor', 'skole' ),
			'slug'     => 'elementor',
			'required' => true,
			'category' => 'required',
			'version'  => '2.7.4',
		...
		array(
			'name'     => esc_html__( 'Vamtam Elementor Integration', 'skole' ),
		...
		array(
			'name'     => esc_html__( 'Vamtam Importers (E)', 'skole' ),
		...
		array(
			'name'     => esc_html__( 'Wordpress SEO', 'skole' ),
			'slug'     => 'wordpress-seo',
			'required' => false,
		...
		array(
			'name'     => esc_html__( 'Ally - Web Accessibility & Usability', 'skole' ),
		...
		array(
			'name'     => esc_html__( 'Really Simple SSL', 'skole' ),
		...
		array(
			'name'     => esc_html__( 'Loco Translate', 'skole' ),
			'slug'     => 'loco-translate',
			'required' => false,
			'category' => 'recommended',
		),
		...
		array(
			'name'     => esc_html__( 'UpDraftPlus', 'skole' ),
			'slug'     => 'updraftplus',
			'required' => false,
			'category' => 'recommended',
		),
		...
		array(
			'name'     => esc_html__( 'LiteSpeed Cache', 'skole' ),
		...
		array(
			'name'     => esc_html__( 'Limit Login Attempts Reloaded', 'skole' ),
		...
		array(
			'name'     => esc_html__( 'Wordfence', 'skole' ),
			'slug'     => 'wordfence',
			'required' => false,
			'category' => 'recommended',
		),
		...
		array(
			'name'     => esc_html__( 'CookieYes | GDPR Cookie Consent & Compliance Notice (CCPA Ready)', 'skole' ),
...
function vamtam_tgmpa_table_columns( $columns ) {
	...
	$columns = array(
		...
		'img'         => '',
		'plugin'      => esc_html__( 'Name', 'skole' ),
		'description' => esc_html__( 'Description', 'skole' ),
		'status'      => esc_html__( 'Status', 'skole' ),
		'version'     => esc_html__( 'Version', 'skole' ),
	);
	return $columns;
}
...
function vamtam_tgmpa_table_data_item( $item, $plugin )
{
	...
	if( isset( $plugin_data['Description'] ) ) {
		...
		$item['description'] =  esc_html__( 'A simple WordPress Plugin.', 'skole' );
	}
	if ( strpos( $plugin['slug'], 'skole' ) !== false ) {
		$item['description'] =  esc_html__( 'Theme Exclusive.', 'skole' );
	}
...
function vamtam_tgmpa_admin_menu_args( $args ) {
	$args['page_title'] = esc_html__( 'Install Plugins', 'skole' );
